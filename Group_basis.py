...
def Basis_clg(e,S,l,n,h1):
	r""" 
	Computes a basis of a group G of the form G=Z/h1*l**(n-1)Z or Z/lZ*(Z/h1*l**(n-2)Z),
	e.g. the ideal class groups of interest
	...
	* n: integer (setting the conductor of the order: l**n)

	* h1: small integer (typically h(O_1))

	OUTPUT:

	* B: a basis B of the group G generated by S as one element if G is cyclic and two elements
	otherwise (the first of order h1*l**(n-2) and the other of order l).

	* DL_S_B: "the DL of elements of B in S", meaning that B[i]=\prod_{j=1}^t S[j]**DL_S_B[i][j].  
	"""

	t=len(S)

	## Choosing g0, a generator whose exponent is the exponent of G
	S0=[s**(l**(n-1)) for s in S]
	S1=[s**h1 for s in S]
	DL_S_B=[[0]*t]
	L_div=divisors(h1)[0:-1]
	for i in range(t):
		good_one=True
		for d in L_div:
			if S0[i]**d==e:
	...
	S_test=[s1**(l**(n-3)) for s1 in S1]
	for i in range(t):
		if S_test[i]**l!=e:
			g0*=S1[i]
			DL_S_B[0][i]+=h1
			return [g0],DL_S_B# At this stage, we know the group is cyclic and we stop
	...
	## Finding g1 of order l to generate the small cyclic factor Z/lZ
	B=[g0]
	L_factors=[(l,n-1)]+list(factor(h1))
	for i in range(t):
		try:
			x=DL(e,B,S[i],L_factors,1)
		except:
			h=S[i]**2
			x=DL(e,B,h,L_factors,1)
			k=-x[0]//2
			g1=S[i]*g0**k
			DL_S_B.append([k*y for y in DL_S_B[0]])
			DL_S_B[1][i]+=1
			B.append(g1)
...
# Obsolete
def DL_matrix_sylow(e,S,B,L_orders):
	r""" 
	Computes the matrix of discrete logarithms of elements of S in the basis B.
	Note that B has to respect the descoposition of the group into p-Sylow subgroups.
	...
	* B: basis respecting the the descoposition of the group into p-Sylow subgroups
...
def DL_matrix(e,S,B,L_factors):
	r""" 
	Computes the matrix of discrete logarithms of elements of S in the basis B.
	The basis does not necessarily respect the decomposition into p-Sylow subgroups.
	...
	* L_orders: list of factors of the cardinality of the group in the form (prime,exponent)
	...
	L_dl=[]
	for i in range(t):
		L_dl.append(DL(e,B,S[i],L_factors,r))
	return matrix(ZZ,L_dl).transpose()
